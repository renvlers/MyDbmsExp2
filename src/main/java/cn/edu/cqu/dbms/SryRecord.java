...
public class SryRecord{
    ...
    public static void operate() throws SQLException{
        if(conn == null)
            ...
            System.out.println("------------------------------------------------------------------");
            System.out.println("请输入您要发放工资的员工ID：");
            int editID = Application.scanner.nextInt();
            Application.scanner.nextLine();
            String staffID = String.valueOf(editID);
            giveSalary(staffID);
            Application.clearScreen();
            System.out.println("工资发放完成！");
            System.out.println("请按回车键返回……");
            Application.scanner.nextLine();
        }
    }

    static void giveSalary(String staffID) throws SQLException{
        String query = "select staffsalary from staff where staffid = " + staffID;
        Statement stmt = conn.createStatement();
        ResultSet rst = stmt.executeQuery(query);
        rst.next();
        String salary = rst.getString("staffsalary");
        LocalDate date = LocalDate.now();
        String currentDate = date.toString();
        ResultSet rst2 = stmt.executeQuery("select count(*) from salary_record");
        rst2.next();
        String salaryID = String.valueOf(rst2.getInt("count") + 1);
        String sql = "insert into salary_record values(" + staffID + ", " + salaryID + ", '" + currentDate + "', "
                + salary + ")";
